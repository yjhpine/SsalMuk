...
- 2026-09-17 후속 사용자 지시: 일반 몬스터는 맵 전체 동시 존재 수에 상한을 둔다. `normalPopulationCap`의 임시 시작값은 300이며 세부 밸런스는 나중에 조정한다. 초기 생성·지속 소환에 같은 상한을 적용하고, 꽉 찼을 때 일반 소환을 적립하지 않는다. 기존 일반 적을 삭제하지 않으며 사망·제거로 자리가 나면 다음 일정부터 생성한다. 공중·보스·경험치는 이 상한 대상이 아니다. 몬스터의 좌우 기울임·점프 보행은 제거하고 이동·피격 표현은 유지한다. 플레이어 보행은 유지한다.
