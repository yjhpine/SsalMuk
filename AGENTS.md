...
- A1~A5의 연결·검증 기반, 모델·맵·길찾기·군집 이동과 런타임 장면을 구현했다. 승인된 A5까지 마친 뒤 다음은 B1 플레이어 AI/FSM이며 새 작업 지시의 범위를 확인한다. 이미 승인한 범위에서는 개발·수정·검증의 재승인을 반복하지 않는다. 진행 중 계정 사용량이 10% 이하로 남으면 사용자에게 알린다.
