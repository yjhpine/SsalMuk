    ...
    public sealed class PersistentWorldViewTests
    {
        ...
        public IEnumerator ViewsAreReusedWhileFarUnitsLootAndInFlightAttractionKeepTheirLogic()
        {
            ...
            try
            {
                ...
                long ground = rig.Spawn(UnitKind.Normal, new DVec2(5, 1), 1000);
                ...
                var flightBefore = moving.Position;
                ...
                Assert.That(rig.Unit(ground).Health, Is.EqualTo(999));
        ...
        [UnityTest]
        public IEnumerator DepartedAirReturnsItsViewWithoutLeavingARewardOrOldLease()
        {
            var go = new GameObject("AirDepartureViewFixture");
            try
            {
                using var rig = RunTestRig.Create(enableAi: false);
                var view = go.AddComponent<WorldView>(); view.Initialize(Resources.Load<GameCatalog>("Bootstrap/GameCatalog"));
                var presenter = new WorldPresenter(view);
                long air = rig.Spawn(UnitKind.Air, new DVec2(10, 1), 1000);
                presenter.Refresh(rig.Run);
                var old = view.UnitViews.Single(x => x.UnitId == air); var lease = old.Lease;
                rig.Advance(2); presenter.Refresh(rig.Run);
                Assert.That(rig.World.Units.TryGet(air, out _), Is.False);
                Assert.That(view.UnitViews.Any(x => x.UnitId == air), Is.False);
                Assert.That(old.Accepts(lease), Is.False);
                Assert.That(view.VisibleUnitCount, Is.EqualTo(1));
