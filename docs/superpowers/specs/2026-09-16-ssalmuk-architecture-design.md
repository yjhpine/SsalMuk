...
- 2026-09-17 추가 지시로 일반 몬스터의 전체 동시 존재 수를 `normalPopulationCap`으로 제한한다(임시 300). 기존 일반 적을 회수하는 방식이 아니라 초기 생성과 지속 소환을 제한한다. `SpawnDirector`는 등록·제거 이벤트로 화면 밖을 포함한 일반 수를 추적한다. 막힌 일반 생성의 예약도 남은 자리만큼만 유지하며, 상한 중 지난 일반 일정은 적립하지 않는다. 보스·공중 일정과 경험치는 이 상한과 독립이다.
...
경로 계산 중에는 직전 경로의 유효한 부분만 사용한다. 공유 거리장은 이전 유효 거리장을 반경별로 보관해 목표 셀 변경 중에도 안전한 방향을 이어 쓴다. 지형 버전이 바뀌면 이전 거리장을 폐기한다. 먼 적의 진행 중인 요청은 목표가 다른 셀로 움직였다는 이유만으로 매번 취소하지 않고, 계산 결과를 받아 기존 경로로 이동하면서 새 목표를 탐색한다. 길이 끊기면 장애물 앞에서 멈추고 재탐색한다. 계산 지연을 이유로 구조물을 통과하지 않는다. 공간 해시는 최근접·주변 적·경험치·접촉·공격 후보를 조회하며, 실제 형상 판정은 후보에 대해 별도로 수행한다. 최근접 검색은 청크 경계나 카메라 경계에서 임의 종료하지 않고 더 가까운 대상이 없다는 거리 하한으로 종료한다.
