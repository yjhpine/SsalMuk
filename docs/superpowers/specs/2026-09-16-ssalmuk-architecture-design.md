...
| 일반 몬스터 | 플레이어가 있는 지역의 공유 거리장 | 내리막 방향 + 가까운 지상 적에 대한 약한 이동 방향 회피 |
...
지상 몸체는 원으로 취급한다. 2026-09-17 후속 승인에 따라 지상끼리 겹침을 반복적으로 분리하던 Resolve와 접선 후보 평가를 제거한다. 2026-09-18 최적화에서는 각 고정 주기 시작 위치로 지상 적의 셀별 개수·중심·최대 반경을 집계하고 주변 최대 7×7셀의 근사 회피 벡터를 의도 이동에 약하게 섞는다. 저장소를 재사용하며 밀집한 셀의 모든 개체를 유닛마다 다시 열거하지 않는다. 공격·접촉·최근접 조회에는 이 근사를 적용하지 않는다. 최종 이동 길이는 원래 의도 이동 이하로 제한한다. 다른 개체 위치를 직접 바꾸지 않으며 정지한 개체·플레이어·공중에는 군중 회피를 적용하지 않는다. 공격 피격 넉백은 이 방향 회피 이후에 더해 기존 힘을 유지한다. 최종 이동은 지형 쓸기·슬라이드로 처리하므로 구조물 통과나 순간이동을 허용하지 않는다. 겹침 자체는 유효한 상태다.

일반·보스의 이동 방향은 기본 약 0.1초마다 개체별로 분산 갱신하고, 위치 이동·FSM·지형 충돌은 매 고정 주기에 유지한다. 목표 셀/지형 버전 변경, 새 경로 완성, 경로 지점 접근, 실제 장애물에 막힘, 넉백은 방향 캐시를 다시 평가하도록 한다. 플레이어 low AI와 공중 직진은 이 판단 주기의 대상이 아니다.

`UnitDefinition.BodyRadius`는 지형·접촉 판정, `HurtRadius`는 근접 공격·투사체·폭발의 피격 판정, `VisualFootOffset`은 표시 발 위치다. 표시 스프라이트의 짧은 변을 원형 기준 지름으로 삼아 실제 에셋에 BodyRadius=지름×0.4, HurtRadius=지름×0.55를 기록하고 이전 발 위치를 보존한다. 타격 후보 조회에는 LargestHurtRadius를 포함해 몸통 밖의 유효한 명중 대상을 빠뜨리지 않는다. 난이도·진단용 정의 복사도 세 값을 보존한다. Core는 Unity 스프라이트를 참조하지 않는다.
...
| 플레이어 ↔ 구조물 | 막힘 | 없음 |
| 지상 적 ↔ 구조물 | 막힘 | 없음 |
| 지상 적 ↔ 지상 적 | 겹침 허용, 이동 방향만 약하게 회피 | 서로에게 없음 |
