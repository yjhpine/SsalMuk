...
지상 몸체는 원으로 취급한다. 2026-09-17 후속 승인에 따라 지상끼리 겹침을 반복적으로 분리하던 Resolve와 접선 후보 평가를 제거한다. 자발적 이동 중인 지상 적만 인접 후보를 한 번 조회하고, 겹침 비율로 가중한 회피 벡터를 의도 이동에 약하게 섞는다. 최종 이동 길이는 원래 의도 이동 이하로 제한한다. 다른 개체 위치를 직접 바꾸지 않으며 정지한 개체·플레이어·공중에는 군중 회피를 적용하지 않는다. 공격 피격 넉백은 이 방향 회피 이후에 더해 기존 힘을 유지한다. 최종 이동은 기존 지형 쓸기·슬라이드로 처리하므로 구조물 통과나 순간이동을 허용하지 않는다. 겹침 자체는 유효한 상태다.
