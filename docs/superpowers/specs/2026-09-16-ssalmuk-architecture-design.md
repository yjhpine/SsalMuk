...
- 작성일: 2026-09-16
- 상태: 구조 설계 검토본 · 게임 코드 및 Unity 연결 작업 전
- 기준: [게임 기획서](../../planning/GAME_DESIGN.md), [개발 하네스 설계](../../development/HARNESS.md)
- 적용 프로젝트: `C:/Users/ace21/Desktop/SsalMuk`, Unity `6000.4.6f1`

이 문서는 기획을 구현할 책임 분리, 상태 전환, 데이터 흐름과 검증 경계를 정한다. 아래 클래스·폴더·도구는 구현할 설계이며, 현재 존재하거나 실행 검증을 통과했다는 의미가 아니다. 기존에 확정한 게임 규칙을 유지하고, 이번 구조 설계에서 선택한 구현 방식은 검토 대상으로 제시한다.

## 1. 선택한 방향과 대안

권장안은 **C# 전투 모델 + Unity 표현부 + MVP**다. 모든 몬스터와 경험치를 보존하므로 전투 데이터를 화면의 GameObject 생명주기와 분리한다. 게임 규칙은 C# 모델에서 계산하고 Unity는 씬 전환, 입력 UI, 스프라이트와 효과를 담당한다.

| 접근 | 장점 | 이 프로젝트에서의 부담 | 판단 |
| --- | --- | --- | --- |
| C# 모델 + Unity 표현 + MVP | 화면 밖 데이터 보존, 규칙 검증, 밸런스 변경의 경계가 명확함 | 원형 유닛과 격자 장애물의 충돌·공격 판정을 직접 구현해야 함 | 채택 제안 |
...
- 경험치는 원래 위치·값·ID를 유지한다. 복귀 시 남아 있어야 하며 이동 거리로 자동 회수하지 않는다.
...
지상 몸체는 원으로 취급한다. 의도 속도와 넉백을 합친 이동 구간을 장애물에 쓸기 검사하고 접선 방향으로 미끄러지게 한다. 지상 유닛끼리는 겹침을 분리하며 고정된 ID 순서·설정된 반복 수로 밀집을 완화한다. 밀집 보정 뒤에도 구조물 침범을 다시 막는다. 해결되지 않은 밀집은 그 자리에서 정체될 수 있으며 순간이동으로 해소하지 않는다.
...
6. 살아 있는 플레이어의 경험치 수집·레벨업·선택 기회를 처리한다.
7. 읽기 전용 상태와 이벤트를 표시부로 전달한다.

동일 주기에서 플레이어가 먼저 처치한 적은 이후 접촉 피해를 주지 않는 제안이다. 같은 주기의 복수 접촉은 충돌 시점과 ID로 순서를 정하고, 처음 수락한 피해가 즉시 짧은 무적을 시작하게 한다. 무적 중 거절된 후보는 빨간 점멸·팽창·넉백을 다시 발생시키지 않는다.
...
이번 설계의 공격 기본안은 **구조물이 이동만 막고 무기 공격은 통과**하는 것이다. 구조물에는 체력·파괴 처리를 두지 않는다. 파이어볼은 적 명중 외에는 정의된 비행 수명이 끝날 때 소멸하며, 소멸만으로 폭발 피해를 만들지 않는다. 이는 아직 기획에서 정하지 않았던 판정·정리 방식의 제안이다.
...
`SpawnDirector`는 생존 시계 하나를 기준으로 일반·공중·보스의 독립 일정을 관리한다. 난이도 곡선은 시간에 따른 생성량과 생성 시 기본 스펙을 정한다. 이미 살아 있는 적의 체력을 난이도 상승 때 갑자기 회복시키지 않도록 생성 시 스펙을 고정하는 안이다.
...
- 공중은 일정마다 화면 한쪽을 시드 난수로 고르고 한 무리를 생성한다. 무리 기준점에서 출현 당시 플레이어 위치로 향하는 방향 하나를 고정해 평행하게 통과시키는 안이다. 이후 플레이어를 유도 추적하지 않는다.
...
도입은 실행·검증 토대 → 생성·맵·이동 → AI·피격·검 전투 → 경험치·보상·나머지 무기 → 무한 유지·스폰·연출 통합 순서로 나눈다. 모든 단계는 최종 구조를 향한 부분 구현으로 표시하며, 첫 검 전투가 된 시점을 전체 게임 완료로 처리하지 않는다. 세부 구현 계획은 이 설계 검토 뒤 작성한다.

## 15. 검토 지점과 완료 기준

이번 설계의 선택은 MVP, Factory Method, Flyweight, 모델과 표시 분리, 격자·원형 판정, FSM·길찾기 방식이다. 원거리 경험치와 모든 몬스터의 보존은 이번 대화에서 확정한 규칙이다.

공격의 구조물 통과, 파이어볼 비행 수명, 발동당 피해 중복 기준, 공중 무리 평행 진행, 기존 몬스터의 생성 시 스펙 유지와 동일 주기의 피해 순서는 상세 동작 제안으로 명시했다. 수정 요청이 없더라도 문서 존재만으로 구현 승인을 받았다고 처리하지 않는다.
