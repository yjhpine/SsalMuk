...
- **경험치와 일반·보스 몬스터를 계속 유지한다.** 2026-09-17 사용자 정정에 따라 공중 무리만 직선 돌진 후 화면 밖 여유 거리에서 무보상 퇴장한다. 진입·넉백 중에는 유지한다.
...
- 2026-09-18 정정: 개수는 강화당 좌우 교대 1개, 연속 공격 강화만 최대 2레벨(총 3연타)이다. 나머지 강화와 선택 기회에 임의 상한을 넣지 않는다. 표현 범위 초과·처리 지연을 숨은 Clamp나 공격 누락으로 처리하지 않는다.
...
추가 사용자 지시를 A3의 경험치 상태, C1의 접촉 획득, D2의 아트/이펙트/발광 구슬, H29~H30 검증에 연결했다. 기존의 반경 진입 즉시 획득 계획은 비행 후 접촉 획득으로 변경한다. 최종 캐릭터 스프라이트와 공격 이펙트 도입은 D2에서 진행한다. A5에는 기능 확인용 단색 이미지와 라이선스를 포함한 한글 UI 글꼴을 적용했다.

## 2. 계획 작성 당시 출발점과 현재 진행

2026-09-17 A~D 기본 기능과 통합 도구·Windows 플레이 빌드를 구현했다. low AI 자동 운영, 네 무기와 무한 성장, 보상 UI, 지속 출현, 아트·연출·풀을 연결했다. 사용자가 현재 검사를 종료하고 다음 단계로 진행하도록 지시했으므로 D3 추가 검사를 종료한다. 10분 누적은 통과했고 30분은 기한 초과로 미완료다. 최신 증거와 미검증 범위는 [D3 결과](../../development/D3_VALIDATION.md)를 따르며, 다음은 [실제 플레이·밸런스 조정](../../development/PLAYTEST_TUNING.md)이다.
...
| D | [무한 스폰·연출·통합 검증](2026-09-16-ssalmuk-d-integration.md) | 공중 무리·보스·일반/보스/경험치 보존·연출·빌드 실행까지 연결 | C |
...
검증 진입점 A1에서 아래 형식을 사용한다. 아직 구현하지 않은 fixture와 Smoke/Stress 시나리오는 성공한 검증으로 기록하지 않는다.
...
2026-09-17 후속 승인: 모든 유닛의 바닥 그림자를 제거하고 일반 상한을 500으로 변경한다. Unity 연결 확인 후 실제 표시를 포함한 300·500마리의 짧은 밀집 비교를 수행한다. 군중 계산 구조·파이어볼 규칙·세부 밸런스의 추가 변경은 비교 결과를 논의한 뒤 정한다. 이 작업으로 D3 장시간 검사나 새 빌드를 재개하지 않는다.

- [x] A1~A5: 실행·맵·군집 장면을 확인하고 단계 커밋 완료.
- [x] B1~B4: low AI·검 전투·사망·재시작을 확인하고 단계 커밋 완료.
- [x] C1~C3: 선택 기회 누적·무기별 성장·무기 4종을 확인하고 단계 커밋 완료.
- [x] D1~D3 구현 및 지금까지 결과 저장. 사용자 지시로 추가 검사 종료; 30분 검증 완료를 뜻하지 않는다.
- [x] H01~H30의 실제 결과·미검증 항목·성능 측정 범위를 D3 결과 문서에서 구분했다.
