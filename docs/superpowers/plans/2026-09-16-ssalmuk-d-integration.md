...
**Spec:** [구조 설계](../specs/2026-09-16-ssalmuk-architecture-design.md) 6·12~15절, [하네스](../../development/HARNESS.md), [전체 계획](2026-09-16-ssalmuk-implementation-plan.md), [선행 C](2026-09-16-ssalmuk-c-growth-weapons.md).
...
- [ ] 검·창·도끼·폭발은 AttackShapeSnapshot과 같은 형상을 반투명 Mesh/표시로 만든다. 투사체·복사본·연속 공격은 발동 ID에 묶어 표시한다. 사거리 증가가 표시와 판정에 함께 적용되는 화면과 테스트를 확인한다.
- [ ] 플레이어 발 위치 Y로 지상 스프라이트의 그리기 순서를 정해 앞뒤 겹침을 읽기 쉽게 한다. 점프·피격 scale은 그리기 순서 기준점을 바꾸지 않는다. 공중은 별도 표시 층을 사용하지만 피해 대상에서는 제외하지 않는다.
- [ ] View의 진입·이탈 거리를 다르게 두고 먼 표시만 회수한다. 논리 몬스터·경험치 ID·위치·체력은 WorldStore에 남긴다. 화면 밖 큰 범위 공격과 경험치 드롭도 동일 데이터에 적용한다.
...
- [ ] 먼 청크 왕복, 피격 중 표시 회수·재생성, 공격 중 재시작, 이동하지 않는 자동 공격을 실제 화면과 데이터로 검증한다. 참고 이미지의 느낌과 비교하되 단색 임시 그림을 최종 원화로 보고하지 않는다. 통과 후 `feat: add walking hit feedback and persistent-world views`로 커밋한다.
...
- [ ] 관련 EditMode·PlayMode 전체를 실행하고 H01~H28을 실제 테스트/시나리오 결과에 연결한다. 실패한 항목만 관련 구현을 수정해 다시 확인한다. 성능 미달이면 공간 검색·거리장 재사용·표시 묶음을 검토하되 게임 규칙을 바꾸는 근사는 사용자와 별도로 정한다.
