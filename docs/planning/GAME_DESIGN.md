...
- 작성일: 2026-09-16
- 상태: 기획 정리 완료 · 구조 설계 검토 중
- 작업 프로젝트: `C:\Users\ace21\Desktop\SsalMuk`
- 진행 순서: 게임 기획 정리 → 구조 설계 → Unity 연결 및 개발·수정·검증

이 문서는 게임의 동작과 플레이 경험을 정한다. 디자인 패턴, 아키텍처, FSM, 길찾기와 코드 구조의 제안은 [구조 설계](../superpowers/specs/2026-09-16-ssalmuk-architecture-design.md)에, 개발·수정·검증 환경은 [개발 하네스 설계](../development/HARNESS.md)에 정리한다. **확정된 게임 규칙**, **구조 설계의 제안**, **나중에 조정할 수치**를 구분한다. 구조 문서의 작성은 실제 구현·연결·검증 완료를 의미하지 않는다.
...
- 멀리 벗어난 구역의 경험치와 모든 몬스터를 계속 유지한다. 거리 때문에 일반·공중·보스를 삭제하거나 경험치를 없애지 않는다.

플레이어와 지상 몬스터는 구조물을 우회해야 한다. 공중 몬스터는 구조물도 무시하고 직선으로 통과하며, 구조물 때문에 이동 경로를 우회하지 않는다.

화면 밖 경험치와 몬스터 보존은 구조 설계 대화에서 확정했다. 사망한 몬스터, 수집한 경험치, 종료한 판은 정상 규칙에 따라 정리한다. 생성 구역의 크기, 표시 객체의 회수 방식, 경로 탐색의 구체적인 방법은 보존 규칙을 유지하는 구조로 설계한다.

## 10. 구조 설계와 밸런스 조정으로 넘길 항목

핵심 플레이 규칙은 위에 정리했다. 아래 항목은 구조 설계 또는 밸런스 조정에서 구체화하며, 구체적인 알고리즘이나 수치가 이미 확정된 것으로 취급하지 않는다.

| 항목 | 후속 단계에서 결정할 내용 |
| --- | --- |
| 공중 무리의 출현 설정 | 주기적인 무리 출현은 확정. 출현할 화면 가장자리의 선정 방식과 구체적인 간격·마릿수 조정 |
...
| 같은 자료 5쪽 | 제시된 디자인 패턴 최소 1~2개, 아키텍처 1개 적용 및 설명 | 구조 설계에서 Factory Method·Flyweight·MVP 적용을 제안. 구현 전 검토 단계 |
...
자료에 제시된 MVP·MVC·MVVM 중 MVP를 구조 설계의 권장안으로 정리했다. 실제 적용 완료를 뜻하지 않으며, 자료의 실습 지시를 현재 Unity 씬을 즉시 수정하라는 실행 지시로 사용하지 않는다.
...
구체적인 도구 구성, 검증 결과 계약, 기록 위치와 코드·씬 수정 방식은 [개발 하네스 설계](../development/HARNESS.md)에 정리했다. 실제 도구 구현과 Unity 연결 확인은 후속 단계다.
...
| 화면 밖 구역과 복귀 | 경험치의 위치·값과 모든 몬스터의 상태를 보존. 거리로 일반·공중·보스를 제거하지 않음 |
