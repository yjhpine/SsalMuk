...
몸통과 피격 영역을 분리한다. 원형 판정의 기준 지름은 표시 스프라이트의 짧은 변이며 몸통 지름은 80%, 몬스터 피격 지름은 110%다. 몸통은 지형 충돌·길찾기·플레이어 접촉 피해에 사용하고, 더 큰 피격 영역은 검·창·도끼·파이어볼 명중과 폭발 적중에 사용한다. 무기의 공격 범위·궤적·피해는 유지하며 넓어진 피격 영역 때문에 가장자리 명중은 늘어날 수 있다. 시각적 팽창이나 보행이 판정 크기를 바꾸지 않는다. 기존 발 위치도 별도로 보존한다.
