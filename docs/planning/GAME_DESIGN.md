...
- 보스 이동 속도는 플레이어의 1.05배를 기준으로 하며 현재 임시값은 플레이어 3, 보스 3.15다.
- 이전 보스가 살아 있어도 새 보스를 추가로 생성한다. 여러 보스가 동시에 존재할 수 있다.
- 보스를 처치해도 게임을 끝내지 않고 무한 생존을 이어간다.

일반·거대 몬스터는 스프라이트가 서로 겹치며 밀집할 수 있다. 2026-09-17 후속 승인으로 겹침을 강제로 밀어내는 충돌 보정을 없애고, 이동 중 가까운 지상 적을 약하게 피하는 방향 보정만 둔다. 정지 상태의 겹침 때문에 밀려나거나 순간이동하지 않는다. 공중 몬스터는 지상 몬스터의 밀집과 구조물을 무시하고 직선으로 통과한다. 플레이어와 적이 겹칠 때의 접촉 피해와 공격에 맞았을 때의 넉백은 유지한다.
...
| 몬스터 밀집 | 지상 몬스터는 겹침을 허용하고 이동 방향만 약하게 회피. 강제 밀어내기 없음. 공중은 이를 통과 |
