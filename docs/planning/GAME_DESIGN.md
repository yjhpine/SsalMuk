...
일반·거대 몬스터는 서로 부딪히며 밀집할 수 있다. 공중 몬스터는 이 밀집에 막히지 않고 통과한다. 공중 몬스터의 플레이어 접촉 피해는 유지한다.
...
- 구조물은 통과할 수 없고 공격으로 파괴되지 않는 장애물이다.
- 플레이어와 몬스터의 이동에는 길찾기를 적용한다.
- 바닥에 떨어진 경험치를 수집해 레벨업한다.

플레이어와 지상 몬스터는 구조물을 우회해야 한다. 공중 몬스터에게 구조물 통과 예외를 둘지는 아직 결정하지 않았다. 공중 몬스터가 지상 몬스터의 밀집을 무시한다는 규칙과 구조물 충돌 규칙은 구분한다.
...
| 항목 | 현재 상태 / 검토할 제안 |
| --- | --- |
| 공중 몬스터와 구조물 | 구조물은 통과·파괴 불가로 확정. 공중 몬스터에게 통과 예외를 둘지는 별도 결정 |
...
| 공중 무리의 출현 | 출현 간격·방향·마릿수와 구조물 충돌 규칙을 별도 결정 |
...
| 구조물과 이동 | 플레이어와 지상 몬스터는 구조물을 통과하거나 파괴하지 않고 우회 |
