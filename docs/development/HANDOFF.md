...
## 최신 상태: 2026-09-22 공통 추적·보스 돌진

- `fc6318a`의 깨끗한 작업 트리에서 시작했다. 모든 적은 매 이동 주기 시작의 플레이어 위치·생존·속도를 `IPlayerPosition`으로 공유한다. 일반·보스의 개별 경로 탐색, 0.1초 방향 캐시, 군중 회피 호출을 제거했다. 지형에는 정지·슬라이드하며 스스로 우회하지 않는다. 플레이어 low AI·A*, 접촉·넉백, 공중 원래 방향·퇴장은 유지한다. 기존 탐색·군중 클래스는 독립 검증용으로 남아 있으나 몬스터 이동에서는 호출하지 않는다.
- 실제 카탈로그에서 일반·공중은 플레이어 속도 1.1배(3.3), 보스 추적은 1.2배(3.6)다. 보스는 0.5초 정지·빨간 예고 후 2배 속도(6)로 돌진한다. 예고 시 방향·끝점을 고정하고 길이는 당시 보스-플레이어 거리의 2배다. 30% 확률로 한 번만 추가 돌진하며 새 예고를 거친다. 사용자 답변에 따라 묶음 뒤 5초간 추적한다. 지형 차단·넉백·사망은 진행 중 돌진을 취소한다. 난수는 판 시드·보스 ID로 분리했다.
- 최종 소스 `f017b3ecfad9b8b5b9814533ed65444e436fa82f9f4044cc704b2cc921f72fde`에서 관련 EditMode 74개·PlayMode 4개가 통과했다. 15개 실행의 manifest·receipt·XML을 같은 소스로 다시 대조한 기록은 `Logs/Validation/shared-chase-boss-20260922/verified-results.json`이다. 공통 위치 `2ce4891681a442669d2de000395d2b8f`, 돌진 경계·확률·충돌 `a27b418e86764b31b8822fe8360d2242`, 실제 예고 표시 `8886afe3200c4d3dbd2852b0200a1200`을 포함한다. 수정 전 재현 4개 실패는 `31702a72d2224fb9818280573b34b3f3`이다.
- 실제 붉은 범위 화면 `Logs/Validation/BossCharge/9a228d35f5ea486fb1a20cbe672cc66d/telegraph.png`를 확인했다. 예고 길이·폭, 0.5초 뒤 숨김, 실제 돌진 이동·종료 정리도 검증했다. 기존 이동·공중·길찾기·플레이어 AI·접촉·몸통 분리·무기 전투·선택 중 전투 검사는 통과했다. Editor는 MainMenu, Play 종료, 컴파일 종료, 미저장 씬 없음이다.
- 500마리 이동 진단 전 `de1a0f7cc4934eef9778fde2c834bd8a`, 후 `c2b2e8aef2c44d6fb850079dc833010a`. 빈 맵, 일반 속도 1.5를 고정해 이전 조건을 보존했고 예열 40주기/측정 350주기(7초), 같은 시드·간격·카메라·몸통을 사용했다. 모델 평균 4.341→0.707ms, P95 5.522→1.031ms, 표시 평균 3.551→3.338ms, 렌더 간격 평균 9.744→7.215ms였다. 개별 회피 제거로 실제 배치는 달라지며 활성 렌더러는 512→510, 유닛 표시는 양쪽 최소 501개다. 각 1회인 Editor 이동 전용 결과이며 전투·플레이어 AI·스폰·경험치는 제외했다. 전체 플레이 FPS나 장시간 성능 보증이 아니다. 원시 전후 보고서는 같은 요약 폴더의 `comparison.json`에 있다.
- 이번 변경은 로컬 소스에 저장하며 원격 푸시·새 Windows 빌드·전체 검사·장시간 검사를 하지 않았다. 기존 2026-09-18 ZIP에는 새 추적·속도·돌진이 없다. 다음에는 Unity에서 새 판의 압박·돌진 체감을 확인하고, 사용자 요청 시 새 실행 파일을 만든다. 잔여 3% 이하 중단 규칙은 유지한다.

## 이전 상태: 2026-09-18 Windows 빌드
