...
- 갱신일: 2026-09-18 후속 작업. **무기 복사본 15도 부채꼴 배치, 파이어볼 화면 밖 소멸·계산 최적화, AI의 근접 사거리 접근을 적용했다. 보스 속도 3.15·몬스터 몸통 100%·자석 반경 1.95로 조정했고 관련 검사 79개를 통과했다. 실제 일반 상한 500, D3 추가 검사 종료(30분 미완료)는 유지한다.**
- 프로젝트: `C:/Users/ace21/Desktop/SsalMuk`, 브랜치: `codex/unit-foundation`.
- 저장 기준: `8f7a51b`에서 시작한 이번 전투 조정을 관련 문서와 함께 로컬 커밋한다. 앞선 개방형 가구 맵·이동 최적화와 몸통/피격 분리 이력은 보존한다. 원격 푸시는 하지 않는다.
