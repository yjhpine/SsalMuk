...
- 갱신일: 2026-09-17 작업. **몸통 80%·피격 110% 분리와 강제 군중 밀어내기 제거를 구현하고 Unity 300/500/1000 비교를 마쳤다. 1,000 전투 조건은 모델 평균 약 71ms로 성능 미달이다. 실제 일반 상한 500과 D3 추가 검사 종료(30분 미완료)를 유지한다.**
- 프로젝트: `C:/Users/ace21/Desktop/SsalMuk`, 브랜치: `codex/unit-foundation`.
- 저장 기준: 작업 중 생성된 `348e9e7`에 주요 분리·이동 구현이 포함되어 있다. 이 커밋을 보존하고 남은 검증 코드와 결과 문서만 후속 로컬 커밋으로 저장한다.
...
- 최신 작업: BodyRadius·HurtRadius·VisualFootOffset을 분리했다. 원형 기준 지름은 표시 스프라이트의 짧은 변, 몸통 80%·피격 110%이며 기존 발 위치는 유지한다. 근접 무기·파이어볼·폭발과 후보 검색에 큰 피격 반경을 연결했다. 반복 분리·접선 후보 평가는 제거하고 이동 중 약한 회피만 적용한다. 정지 겹침은 밀지 않고 지형·접촉 피해·공격 넉백·공중 규칙은 유지한다.
- 현재 소스 `a6d5c8307a91ad962f03eb64ab59cec31d9760732e7392ebbdc6a0c03a854fc9`에서 관련 기능 55개, 개체 수 비교 4개, 실제 GameStart/초기 생성/AI 경험치/HUD 1개로 60개가 통과했다. 원시 근거는 `Logs/Validation/body-hit-separation-20260917/`, 조건·실행 ID·한계는 [로그](../../log.md)에 있다. 비교 통과를 목표 성능 통과로 해석하지 않는다.
- 300/500/1000 이동만의 모델 평균은 23.51/41.32/70.78ms, 1000 전투 포함은 70.74ms였다. 20ms 고정 갱신에 비해 길며 밀집에서 최대 17회 누적 갱신이 발생한다. 1000개 보존·표시와 기본 무기 발동은 확인했지만 원활한 지원을 보증하지 않는다. 무한 강화·장시간 누적·빌드 FPS도 미검증이다. 다음은 남은 지형 검사·주변 조회/회피 비용을 나눠 계측하고 중복·할당·밀집 조회를 줄이는 작업이다.
