...
- D3 저장 커밋 메시지: `test: add integration scenarios and record validation cutoff`. 직전 D2는 `a65448a`, D1은 `4dbf562`다. 원격 푸시는 하지 않는다.
...
- 마지막 Windows 실행 도구 확장 후 Static·EditMode는 통과했고 PlayMode는 Unity가 배칭 설정을 지연 저장해 `SourceChanged`로 거절했다. 원래 사용자 파일과 메모리의 자동 추가 항목을 복원했다. 새 장시간 Windows 실행·취소 동작은 미검증이다. 이후 실제 게임 코드·아트는 바꾸지 않았다.
