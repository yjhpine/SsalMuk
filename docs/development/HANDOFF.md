...
- 갱신일: 2026-09-17. **A~D 기본 기능 뒤 이동 끊김 보완·일반 몬스터 상한·몬스터 보행 제거를 반영했다. 세부 밸런스 조정은 후속 작업이다. D3 추가 검사 종료(30분 미완료)는 유지한다.**
...
- 최신 작업: 일반 몬스터의 맵 전체 동시 존재 상한을 추가했다. `DevelopmentDefaults.normalPopulationCap` 임시값은 300이며 초기·지속 소환을 함께 제한한다. 화면 밖 일반도 포함하고 기존 적은 유지한다. 꽉 찬 동안 지난 소환을 적립하지 않으며 막힌 일반 생성의 대기도 빈자리 수까지만 유지한다. 공중·보스·경험치에는 이 상한을 적용하지 않는다. 세부 수치 조정은 사용자가 나중에 진행하기로 했다.
