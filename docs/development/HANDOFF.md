...
- 갱신일: 2026-09-17. **유닛 그림자 제거·일반 상한 500을 반영하고 Unity에서 300/500 고정 밀집 비교를 마쳤다. 표시 갱신 비용은 약 12% 감소했지만 이동·군중 계산 병목은 남는다. D3 추가 검사 종료(30분 미완료)는 유지한다.**
...
- 최신 작업: 모든 유닛의 프리팹·런타임 그림자를 제거하고 `DevelopmentDefaults.normalPopulationCap` 임시값을 300에서 500으로 올렸다. 초기·지속 소환을 함께 제한하고 화면 밖 일반도 포함하며 기존 적은 유지한다. 꽉 찬 동안 지난 소환을 적립하지 않고 공중·보스·경험치는 이 상한과 독립이다. 경험치 발광·피격·플레이어 보행은 유지한다.
- 이번 소스 `310c328ec8f2118f392c1bcf8f6bdb9607b19f75e12b5a2c3cce762a05cff637`에서 관련 PlayMode 7개·EditMode 8개가 통과했다. 원시 결과와 연결·에셋 읽기·사용자 변경 보존 증거는 `Logs/Validation/shadow-cap-comparison-20260917/`다. [로그](../../log.md)에 전후 비교, 소스 해시, 실행 ID와 한계를 기록했다. 300/500 조건 모두 20 프레임·350회 이동을 수집했으며 정상 생존 FPS나 새 빌드 성능 보증이 아니다.
- 다음 논의: 사용자는 충돌 밀어내기 제거, 몸통 80%, 몬스터 피격 영역 110%를 제안했다. 110%의 의미까지 확인했고 구조 평가를 보고한다. 현재 BodyRadius가 이동·접촉·무기 피격에 공용이므로 분리와 후보 검색 범위 변경이 필요하다. 군중 구조와 판정 변경은 아직 구현하지 않았다. 다음 승인 범위를 확인하고 이어가며 이 기록을 구현 완료로 읽지 않는다.
