...
- 갱신일: 2026-09-18 작업. **벽 없는 개방형 맵·낡고 기울어진 가구 6종과 지형/방향/군중 계산 최적화를 적용했다. 관련 검사 86개 통과, 실제 일반 상한 500 유지. 1,000마리 전투 조건은 개선됐지만 프레임 저하가 남아 있다. D3 추가 검사 종료(30분 미완료)는 유지한다.**
- 프로젝트: `C:/Users/ace21/Desktop/SsalMuk`, 브랜치: `codex/unit-foundation`.
- 저장 기준: `0718fe4`에서 시작한 이번 개방형 가구 맵·이동 최적화를 관련 문서와 함께 로컬 커밋한다. 앞선 `348e9e7`·`34ee95c`의 몸통/피격 분리와 검증 이력은 보존한다. 원격 푸시는 하지 않는다.
