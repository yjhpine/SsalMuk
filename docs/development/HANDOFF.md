...
- 갱신일: 2026-09-18 성장·웨이브 정정. **개수 강화는 한 번에 하나씩 좌우 교대, 연속 공격 강화는 최대 2레벨로 수정했다. 처치 속도와 남은 적 수에 반응하는 2분 전방위 웨이브와 카메라 확대를 적용했고 관련 검사 75개가 통과했다. 일반 상한 500과 D3 추가 검사 종료(30분 미완료)는 유지한다.**
...
- `HighGrowth` 진단 코드의 기대값도 새 성장 규칙에 맞췄으나 해당 스트레스 시나리오는 실행하지 않았다. 전체 검사·장시간 검사·새 Windows 빌드도 실행하지 않았다. 검증 후 Unity는 미저장 변경 없는 MainMenu에서 재생·컴파일이 정지된 상태다. 다음 작업은 현재 프로젝트의 새 판에서 웨이브 압박과 확대된 시야를 체감하며 세부 수치를 조정하는 것이다. 기존 exe에는 이번 변경이 없다.
